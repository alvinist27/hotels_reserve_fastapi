from datetime import date

from fastapi import APIRouter, Body, Query

from src.api.dependencies import DBDep
from src.exceptions import (
    HotelNotFoundException,
    HotelNotFoundHTTPException,
    RoomNotFoundException,
    RoomNotFoundHTTPException,
)
from src.schemas.rooms import RoomAddRequestSchema, RoomPatchRequestSchema
from src.services.rooms import RoomService

rooms_router = APIRouter(prefix='/hotels', tags=['Rooms'])


@rooms_router.get('/{hotel_id}/rooms')
async def get_rooms(
    hotel_id: int,
    db: DBDep,
    date_from: date = Query(examples=['2024-08-01']),
    date_to: date = Query(examples=['2024-08-10']),
):
    return await RoomService(db).get_filtered_by_time(hotel_id, date_from, date_to)


@rooms_router.get('/{hotel_id}/rooms/{room_id}')
async def get_room(hotel_id: int, room_id: int, db: DBDep):
    try:
        return await RoomService(db).get_room(room_id, hotel_id=hotel_id)
    except RoomNotFoundException:
        raise RoomNotFoundHTTPException


@rooms_router.post('/{hotel_id}/rooms')
async def create_room(hotel_id: int, db: DBDep, room_data: RoomAddRequestSchema = Body()):
    try:
        room = await RoomService(db).create_room(hotel_id, room_data)
    except HotelNotFoundException:
        raise HotelNotFoundHTTPException
    return {'status': 'OK', 'data': room}


@rooms_router.put('/{hotel_id}/rooms/{room_id}')
async def edit_room(
    hotel_id: int,
    room_id: int,
    room_data: RoomAddRequestSchema,
    db: DBDep,
):
    await RoomService(db).edit_room(hotel_id, room_id, room_data)
    return {'status': 'OK'}


@rooms_router.patch('/{hotel_id}/rooms/{room_id}')
async def partially_edit_room(
    hotel_id: int,
    room_id: int,
    room_data: RoomPatchRequestSchema,
    db: DBDep,
):
    await RoomService(db).partially_edit_room(hotel_id, room_id, room_data)
    return {'status': 'OK'}


@rooms_router.delete('/{hotel_id}/rooms/{room_id}')
async def delete_room(hotel_id: int, room_id: int, db: DBDep):
    await RoomService(db).delete_room(hotel_id, room_id)
    return {'status': 'OK'}
